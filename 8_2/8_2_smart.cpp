...
public:
  ...
  // Функция для определения лучшей стены для удаления
...
int main() {
  ...
  castle.setWall(1, 3, EAST);
    ...
    int maxCombinedArea;
    std::pair<int, int> wallPosition;
    castle.findBestWallToRemove(maxCombinedArea, wallPosition);
